...
(function() {
  ...
  ordersHandler = function(event) {
    ...
    if (type === 'click') {
      ...
      if (nodeName === 'BUTTON' && target.type === 'button' && (dataset.action != null)) {
    ...
    if (type === 'submit') {
      ...
      switch (target.name) {
        case 'filter':
          return filterOrders(target.filter.value);
        case 'sell':
          return soldOrderItem(target);
        default:
  ...
  soldOrderItem = function(form) {
    var order, orderItem, ref1;
    return ref1 = form.dataset, order = ref1.order, orderItem = ref1.orderItem, ref1;
  };
